...
************************************************************************ */
/* $Id: handler.c,v 1.49 2004/03/16 03:02:58 cheron Exp $ */
...
int	apply_damage_reduction(struct char_data * ch, int eq_pos);
									 ...
									 bitvector_t bitv, bool add)
{
	...
	case APPLY_AGILITY:
		GET_AGILITY(ch) += mod;
		break;
	case APPLY_PRECISION:
		GET_PRECISION(ch) += mod;
		break;
	...
	default:
		extended_mudlog(NRM, SYSL_BUGS, TRUE, "Unknown apply adjust %d attempt (%s, affect_modify).", loc, __FILE__);
		break;
...
void affect_total(struct char_data *ch)
{
	...
	/* Make sure certain values are between 0..25, not < 0 and not > 25! */

	i = 10000; /* (IS_NPC(ch) ? 25 : 18); */

	GET_STRENGTH(ch) = MAX(0, MIN(GET_STRENGTH(ch), i));
	GET_AGILITY(ch) = MAX(0, MIN(GET_AGILITY(ch), i));
	GET_PRECISION(ch) = MAX(0, MIN(GET_PRECISION(ch), i));
	GET_PERCEPTION(ch) = MAX(0, MIN(GET_PERCEPTION(ch), i));
	GET_HEALTH(ch) = MAX(0, MIN(GET_HEALTH(ch), 32000));
	GET_WILLPOWER(ch) = MAX(0, MIN(GET_WILLPOWER(ch), i));
	GET_INTELLIGENCE(ch) = MAX(0, MIN(GET_INTELLIGENCE(ch), i));
	GET_CHARISMA(ch) = MAX(0, MIN(GET_CHARISMA(ch), i));
	GET_LUCK(ch) = MAX(0, MIN(GET_LUCK(ch), i));
	GET_ESSENCE(ch) = MAX(0, MIN(GET_ESSENCE(ch), i));
}
...
void char_from_room(struct char_data *ch)
{
	...
	if (ch == NULL || IN_ROOM(ch) == NOWHERE) {
		extended_mudlog(NRM, SYSL_BUGS, TRUE, "NULL character or NOWHERE in %s, char_from_room", __FILE__);
		exit(1);
	}
...
void char_to_room(struct char_data *ch, room_rnum room)
{
	if (ch == NULL || room == NOWHERE || room > top_of_world)
		extended_mudlog(NRM, SYSL_BUGS, TRUE, "Illegal value(s) passed to char_to_room. (Room: %d/%d Ch: %p",
	...
	if (GET_OBJ_VAL(GET_EQ(ch, WEAR_LIGHT), 2))	/* Light ON */
		...
		/* Stop being on watch, defending, dodging, if we moved. */
		if (GET_POS(ch) == POS_WATCHING || GET_POS(ch) == POS_DEFEND || GET_POS(ch) == POS_DODGE)
...
void obj_to_char(struct obj_data *object, struct char_data *ch)
{
	if (object && ch) {
		...
		extended_mudlog(NRM, SYSL_BUGS, TRUE, "NULL obj (%p) or char (%p) passed to obj_to_char.", object, ch);
}
...
void obj_from_char(struct obj_data *object)
{
	...
	if (object == NULL) {
		extended_mudlog(NRM, SYSL_BUGS, TRUE, "NULL object passed to obj_from_char.");
		return;
	}
...
int	invalid_size(struct char_data *ch, struct obj_data *obj)
{
	if (GET_SIZE(ch) < GET_OBJ_SIZE(obj) - 1 && GET_OBJ_SIZE(obj) != SIZE_NONE)
		return (TRUE);
	if (GET_SIZE(ch) > GET_OBJ_SIZE(obj) && (GET_OBJ_TYPE(obj) == ITEM_WORN || GET_OBJ_TYPE(obj) == ITEM_ARMOR) && GET_OBJ_SIZE(obj) != SIZE_NONE)
		return (TRUE);
	return (FALSE);
}
...
int	newbie_check(struct char_data *ch, struct obj_data *obj)
{
	...
	if (playing_time.day > 0) {
		i = get_obj_in_list_vis(ch, "newbie", NULL, obj);
		if (i)
			return TRUE;
	}
	return FALSE;
...
void equip_char(struct char_data *ch, struct obj_data *obj, int pos)
{
	...
	if (GET_EQ(ch, pos)) {
		extended_mudlog(NRM, SYSL_BUGS, TRUE, "Char is already equipped: %s, %s", GET_NAME(ch),
			obj->short_description);
		return;
	}
	if (obj->carried_by) {
		extended_mudlog(NRM, SYSL_BUGS, TRUE, "EQUIP: Obj is carried_by when equip.");
		return;
	}
	if (IN_ROOM(obj) != NOWHERE) {
		extended_mudlog(NRM, SYSL_BUGS, TRUE, "EQUIP: Obj is in_room when equip.");
		return;
	}
	if (invalid_align(ch, obj) || invalid_race(ch, obj) || newbie_check(ch, obj)) {
	...
	if (GET_OBJ_TYPE(obj) == ITEM_ARMOR) {
		GET_PD(ch) += apply_pd(ch, pos);
		GET_REDUCTION(ch) += apply_damage_reduction(ch, pos);
	}
	...
	if (IN_ROOM(ch) != NOWHERE) {
		...
		extended_mudlog(NRM, SYSL_BUGS, TRUE, "IN_ROOM(ch) = NOWHERE when equipping char %s.", GET_NAME(ch));
...
struct obj_data *unequip_char(struct char_data *ch, int pos)
{
	...
	if (IN_ROOM(ch) != NOWHERE) {
		...
		extended_mudlog(NRM, SYSL_BUGS, TRUE, "IN_ROOM(ch) = NOWHERE when unequipping char %s.", GET_NAME(ch));
...
/* Search a given list for an object virtual number, and return a ptr to that obj */
struct obj_data *get_obj_in_list_vnum(int vnum, struct obj_data *list)
{
	struct obj_data *i;

	for (i = list; i; i = i->next_content)
		if (GET_OBJ_PROTOVNUM(i) == vnum)
			return (i);
...
void obj_to_room(struct obj_data *object, room_rnum room)
{
	if (!object || room < 0 || room > top_of_world)
		extended_mudlog(NRM, SYSL_BUGS, TRUE, "Illegal value(s) passed to obj_to_room. (Room #%d/%d, obj %p)",
...
void obj_from_room(struct obj_data *object)
{
	...
	if (!object || IN_ROOM(object) == NOWHERE) {
		extended_mudlog(NRM, SYSL_BUGS, TRUE, "NULL object (%p) or obj not in a room (%d) passed to obj_from_room",
...
void obj_to_obj(struct obj_data *obj, struct obj_data *obj_to)
{
	...
	if (!obj || !obj_to || obj == obj_to) {
		extended_mudlog(NRM, SYSL_BUGS, TRUE, "NULL object (%p) or same source (%p) and target (%p) obj passed to obj_to_obj.",
	obj, obj, obj_to);
		return;
	}
...
void obj_from_obj(struct obj_data *obj)
{
	...
	if (obj->in_obj == NULL) {
		extended_mudlog(NRM, SYSL_BUGS, TRUE, "(%s): trying to illegally extract obj from obj.", __FILE__);
		return;
	}
	obj_from = obj->in_obj;
...
void extract_obj(struct obj_data *obj)
{
	...
	if (obj->worn_by != NULL)
		if (unequip_char(obj->worn_by, obj->worn_on) != obj)
			extended_mudlog(NRM, SYSL_BUGS, TRUE, "Inconsistent worn_by and worn_on pointers!!");
	if (IN_ROOM(obj) != NOWHERE)
		obj_from_room(obj);
...
void extract_char_final(struct char_data *ch)
{
	...
	if (IN_ROOM(ch) == NOWHERE) {
		extended_mudlog(NRM, SYSL_BUGS, TRUE, "NOWHERE extracting char %s. (%s, extract_char_final)",
	...
	if (ch->desc) {
      ...
      if (STATE(ch->desc) != CON_GET_BECOME) {
        STATE(ch->desc) = CON_MENU;
        write_to_output(ch->desc, TRUE, MENU);
      }
		}
	}
	...
	/* cancel skills and spells */
	for (i = 0; i < 3; i++) {
		if (GET_PLAYER_EVENT(ch, i)) {
			event_cancel(GET_PLAYER_EVENT(ch, i));
			GET_PLAYER_EVENT(ch, i) = NULL;
		}
	}

	remove_char_guild_data(ch); /* Remove the char's guild info */
...
/* Extract a ch minimally from the world for soft deaths */
void extract_char_soft(struct char_data *ch)
{
	struct char_data *k, *temp;
	...
	if (IN_ROOM(ch) == NOWHERE) {
		extended_mudlog(NRM, SYSL_BUGS, TRUE, "NOWHERE extracting char %s. (%s, extract_char_soft)",
				GET_NAME(ch), __FILE__);
		exit(1);
	}

	/*
	 * Sanity check... mobs should never get extracted all softy like
	 * Cheron, 2003-06-10
	 */
	 if (IS_NPC(ch)) {
		 extract_char(ch);
		 extended_mudlog(NRM, SYSL_BUGS, TRUE, "attempt to soft extract npc %s. (%s, extract_char_soft)",
			 GET_NAME(ch), __FILE__);
		 return;
	 }
...
void extract_pending_chars(void)
{
	...
	if (extractions_pending < 0)
		extended_mudlog(NRM, SYSL_BUGS, TRUE, "Negative (%d) extractions pending.", extractions_pending);
	...
	if (extractions_pending > 0)
		extended_mudlog(NRM, SYSL_BUGS, TRUE, "Couldn't find %d extractions as counted.", extractions_pending);
...
struct char_data *get_char_room_vis(struct char_data *ch, char *name, int *number, int ooc)
{
	 ...
	 * If we want OOC names (int ooc)
	 * If the character is IC and has keywords, we search those.
	 * If the character is OOC or a mob, we search name.
	 * Torgny Bjers (artovil@arcanerealms.org), 2003-01-01
	 */
	/*
	 * Addendum: If the target is recognized, we search name.
	 * Catherine Gore (cheron@arcanerealms.org, 2003-01-09
	 * Bloody men.
	 */

	for (i = world[IN_ROOM(ch)].people; i && *number; i = i->next_in_room)
		if (((IS_OOC(i) || IS_NPC(i) || ooc || find_recognized(ch, i)) && isname(name, i->player.name)) || (GET_KEYWORDS(i) && !IS_NPC(i) && IS_IC(i) && isname(name, GET_KEYWORDS(i))))
			if (CAN_SEE(ch, i))
				if (--(*number) == 0)
					return (i);
...
struct char_data *get_char_world_vis(struct char_data *ch, char *name, int *number, int ooc)
{
	...
	if ((i = get_char_room_vis(ch, name, number, ooc)) != NULL)
		return (i);
	...
	for (i = character_list; i && *number; i = i->next) {
		if (IN_ROOM(ch) == IN_ROOM(i))
			continue;
		// If they are OOC, or we want ooc names, or is recognized, we search name.
		if ((IS_OOC(i) || IS_NPC(i) || ooc || find_recognized(ch, i)) && !isname(name, i->player.name))
			continue;
		// If they are IC, and we don't want ooc names, we search keywords.
		if (!ooc && GET_KEYWORDS(i) && !IS_NPC(i) && IS_IC(i) && !isname(name, GET_KEYWORDS(i)))
			continue;
		if (!CAN_SEE(ch, i))
			continue;
		if (--(*number) != 0)
...
struct char_data *get_char_vis(struct char_data *ch, char *name, int *number, int where, int ooc)
{
	if (where == FIND_CHAR_ROOM)
		return get_char_room_vis(ch, name, number, ooc);
	else if (where == FIND_CHAR_WORLD)
		return get_char_world_vis(ch, name, number, ooc);
	else
		return (NULL);
}
		...
		if (i->name)
			if (isname(name, i->name))
				if (CAN_SEE_OBJ(ch, i))
					if (--(*number) == 0)
						return (i);
		if (GET_OBJ_COLOR(i) > 0 && GET_OBJ_COLOR(i) < MAX_COLORS)
			if (isname(name, EXTENDED_COLORS[(int)GET_OBJ_COLOR(i)].name))
				if (CAN_SEE_OBJ(ch, i))
					if (--(*number) == 0)
						return (i);
	}
...
const	char *money_desc(int amount)
{
	...
	if (amount <= 0) {
		extended_mudlog(NRM, SYSL_BUGS, TRUE, "Try to create negative or 0 money (%d).", amount);
		return (NULL);
	}
...
struct obj_data *create_money(int amount)
{
	...
	if (amount == 1) {
		...
		if (amount < 10)
			...
			strcpy(buf, "There are a LOT of coins.");
				 ...
				 struct char_data ** tar_ch, struct obj_data ** tar_obj)
{
	...
	*tar_obj = NULL;
	...
	if (IS_SET(bitvector, FIND_CHAR_ROOM)) {	/* Find person in room */
		if ((*tar_ch = get_char_room_vis(ch, name, &number, 0)) != NULL)
			return (FIND_CHAR_ROOM);
	}
	if (IS_SET(bitvector, FIND_CHAR_WORLD)) {
		if ((*tar_ch = get_char_world_vis(ch, name, &number, 0)) != NULL)
