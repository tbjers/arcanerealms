...
************************************************************************ */
/* $Id: mobact.c,v 1.8 2002/12/10 13:08:30 arcanere Exp $ */
...
#define	MOB_AGGR_TO_RACE (MOB_AGGR_FAE | MOB_AGGR_DIVINE | MOB_AGGR_INFERNAL | MOB_AGGR_HUMAN)
...
void mobile_activity(void)
{
	...
	for (ch = character_list; ch; ch = next_ch) {
		...
		if (MOB_FLAGGED(ch, MOB_SPEC) && !no_specials) {
			if (mob_index[GET_MOB_RNUM(ch)].func == NULL) {
				extended_mudlog(BRF, SYSL_BUGS, TRUE, "%s (#%d): Attempting to call non-existing mob function.",
	...
	(!MOB_FLAGGED(ch, MOB_STAY_ZONE) ||
	 (world[EXIT(ch, door)->to_room].zone == world[IN_ROOM(ch)].zone))) {
		...
		/* Aggressive Mobs */
		if (MOB_FLAGGED(ch, MOB_AGGRESSIVE | MOB_AGGR_TO_ALIGN | MOB_AGGR_TO_RACE)) {
			found = FALSE;
			for (vict = world[IN_ROOM(ch)].people; vict && !found; vict = vict->next_in_room) {
				...
				if (MOB_FLAGGED(ch, MOB_AGGRESSIVE   ) ||
					 ...
					 (MOB_FLAGGED(ch, MOB_AGGR_HUMAN   ) && IS_HUMAN(vict)) ||
					 (MOB_FLAGGED(ch, MOB_AGGR_FAE     ) && IS_FAE(vict)) ||
					 (MOB_FLAGGED(ch, MOB_AGGR_DIVINE  ) && IS_DIVINE(vict)) ||
					 (MOB_FLAGGED(ch, MOB_AGGR_INFERNAL) && IS_DEMONIC(vict))) {
		 ...
		 */
		if (AFF_FLAGGED(ch, AFF_CHARM) && ch->master && num_followers_charmed(ch->master) > (GET_CHARISMA(ch->master) - 2) / 3) {
...
bool aggressive_mob_on_a_leash(struct char_data *slave, struct char_data *master, struct char_data *attack)
{
	...
	dieroll = number(1, 20);
	if (dieroll != 1 && (dieroll == 20 || dieroll > 10 - (GET_CHARISMA(master) / 100) + (GET_INTELLIGENCE(slave) / 100))) {
