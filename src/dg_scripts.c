...
int find_eq_pos_script(struct char_data *ch, char *arg)
{
	...
	struct eq_pos_list {
		...
		{"face",		WEAR_FACE			},
		{"neck",		WEAR_NECK			},
		{"throat1",	WEAR_THROAT_1	},
		{"throat2",	WEAR_THROAT_2	},
		{"wings",		WEAR_WINGS		},
		{"body",		WEAR_BODY			},
		{"about",		WEAR_ABOUT		},
		{"back",		WEAR_BACK			},
		{"outside",	WEAR_OUTSIDE	},
		{"arms",		WEAR_ARMS			},
		{"rwrist",	WEAR_WRIST_R	},
		{"lwrist",	WEAR_WRIST_L	},
		{"hands",		WEAR_HANDS		},
...
char_data	*get_char(char *name)
{
	...
	if (*name == UID_CHAR) {
		...
		if (i && (IS_NPC(i) || (GET_INVIS_LEV(i) == RIGHTS_NONE)))
			return i;
	} else {
...
char_data	*get_char_by_obj(obj_data *obj, char *name)
{
	...
	if (*name == UID_CHAR) {
		...
		if (obj->carried_by &&
			isname(name, obj->carried_by->player.name) && valid_dg_target(obj->carried_by, TRUE))
			return obj->carried_by;
		
		if (obj->worn_by &&
			isname(name, obj->worn_by->player.name) && valid_dg_target(obj->worn_by, TRUE))
			return obj->worn_by;
...
{		 
	...
	if (*name == UID_CHAR) {
		...
		if (ch && (IS_NPC(ch) || (GET_INVIS_LEV(ch) == RIGHTS_NONE)))
			return ch;
	} else {
		for (ch = room->people; ch; ch = ch->next_in_room)
			if (isname(name, ch->player.name) && valid_dg_target(ch, TRUE))
...
void do_stat_trigger(struct char_data *ch, trig_data *trig)
{
	...
	if (!trig) {
		mlog("SYSERR: NULL trigger passed to do_stat_trigger.");
		return;
	}
	...
	write_to_output(ch->desc, FALSE, "%s", sb, 1);
}
...
ACMD(do_attach)	
{
	...
	if (is_abbrev(arg, "mtr")) {
		if ((victim = get_char_vis(ch, targ_name, NULL, FIND_CHAR_WORLD, 1))) {
			if (IS_NPC(victim))  {
	...
	if ((rn != NOTHING) && (trig = read_trigger(rn))) {
		...
		send_to_charf(ch, "Trigger %d (%s) attached to %s [%d].\r\n",
	...
	send_to_char("Players can't have scripts.\r\n", ch);
		...
		if ((object = get_obj_vis(ch, targ_name, NULL))) {
		...
		sprintf(buf, "Trigger %d (%s) attached to %s [%d].\r\n",
...
{	 
	...
	if (!str_cmp(arg1, "room")) {
		...
		if (is_abbrev(arg1, "mob")) {
			if (!(victim = get_char_vis(ch, arg2, NULL, FIND_CHAR_WORLD, 1)))
			...
			if (!(object = get_obj_vis(ch, arg2, NULL)))
			...
			else if ((victim = get_char_room_vis(ch, arg1, NULL, 0)));
			else if ((object = get_obj_in_list_vis(ch, arg1, NULL, world[IN_ROOM(ch)].contents)));
			else if ((victim = get_char_vis(ch, arg1, NULL, FIND_CHAR_WORLD, 1)));
...
void script_log(char *msg)
{
	char buf[256];
	sprintf(buf, "%s", msg);
	extended_mudlog(NRM, SYSL_TRIGGERS, TRUE, "ERROR! %s", buf);
}
	...
	char *str)
{
	...
	if (!str_cmp(field, "strlen")) {                     /* strlen    */
		...
		*str = '\0';
		...
		else strcpy(str, cmd_info[cmd].command);
		return TRUE;
	}
								...
								int type, char *var, char *field, char *subfield, char *str)
{
	...
	char_data *ch = NULL, *c = NULL, *rndm = NULL;
	obj_data *obj = NULL, *o = NULL;
	struct room_data *room = NULL, *r = NULL;
	char *name = '\0';
	int num, count;
	...
	if (!*field) {
		...
		if (c) {
			if (text_processed(field, subfield, vd, str)) return;
			...
			else if (!str_cmp(field, "rights")) {
				sprintf(str, "%d", wiz_int_rights(get_max_rights(c)));
			}
				...
				strcpy(str, race_list[(int)GET_RACE(c)].name);
			...
			else if (!str_cmp(field, "newbie")) {
				if (IS_SET(PLR_FLAGS(c), PLR_NEWBIE))
					strcpy(str, "1");
				else
					strcpy(str, "0");
			}

			else if (!str_cmp(field, "in_quest")) {
				if (IS_SET(PRF_FLAGS(c), PRF_QUEST))
					strcpy(str, "1");
				else
					strcpy(str, "0");
			}

			else if (!str_cmp(field, "npc")) {
				if (IS_NPC(c))
					strcpy(str, "1");
				else
					strcpy(str, "0");
				...
				sprintf(str, "%d", GET_STRENGTH(c) / 100);

			else if (!str_cmp(field, "agl"))
				sprintf(str, "%d", GET_AGILITY(c) / 100);
			
			else if (!str_cmp(field, "prc"))
				sprintf(str, "%d", GET_PRECISION(c) / 100);
			
			else if (!str_cmp(field, "per"))
				sprintf(str, "%d", GET_PERCEPTION(c) / 100);
			
			else if (!str_cmp(field, "hea"))
				sprintf(str, "%d", GET_HEALTH(c) / 100);
			
			else if (!str_cmp(field, "wil"))
				sprintf(str, "%d", GET_WILLPOWER(c) / 100);
			
			else if (!str_cmp(field, "int"))
				sprintf(str, "%d", GET_INTELLIGENCE(c) / 100);
			
			else if (!str_cmp(field, "cha"))
				sprintf(str, "%d", GET_CHARISMA(c) / 100);
			
			else if (!str_cmp(field, "lck"))
				sprintf(str, "%d", GET_LUCK(c) / 100);

			else if (!str_cmp(field, "ess"))
				sprintf(str, "%d", GET_ESSENCE(c) / 100);
...
/* see note in dg_scripts.h */
				...
				int pos = find_eq_pos_script(c, subfield);
			...
			if (text_processed(field, subfield, vd, str)) return;
				...
				sprinttype(GET_OBJ_TYPE(o), item_types, str, sizeof(str));
				...
				if (SCRIPT(o)) {
					...
					if (vd)
						...
						*str = '\0';
						sprintf(buf2, 
									 "Trigger: %s, VNum %d. unknown object field: '%s'",
									 GET_TRIG_NAME(trig), GET_TRIG_VNUM(trig), field);
						script_log(buf2);
					}
				} else {
			...
			if (text_processed(field, subfield, vd, str)) return;
				...
				if (r->dir_option[NORTH])
					sprintbit(r->dir_option[NORTH]->exit_info, exit_bits, str, sizeof(str));
				else
					*str = '\0';
			}
			else if (!str_cmp(field, "east")) {
				if (r->dir_option[EAST])
					sprintbit(r->dir_option[EAST]->exit_info, exit_bits, str, sizeof(str));
				else
					*str = '\0';
			}
			else if (!str_cmp(field, "south")) {
				if (r->dir_option[SOUTH])
					sprintbit(r->dir_option[SOUTH]->exit_info, exit_bits, str, sizeof(str));
				else
					*str = '\0';
			} else if (!str_cmp(field, "west")) {
				if (r->dir_option[WEST])
					sprintbit(r->dir_option[WEST]->exit_info, exit_bits, str, sizeof(str));
				else
					*str = '\0';
			}
			else if (!str_cmp(field, "up")) {
				if (r->dir_option[UP])
					sprintbit(r->dir_option[UP]->exit_info, exit_bits, str, sizeof(str));
				else
					*str = '\0';
			}
			else if (!str_cmp(field, "down")) {
				if (r->dir_option[DOWN])
					sprintbit(r->dir_option[DOWN]->exit_info, exit_bits, str, sizeof(str));
				else
					*str = '\0';
			}
				...
				if (room_id != NOWHERE)
					sprintf(str, "%ld", (long) room_id + ROOM_ID_BASE); /* added by welcor */
				else
					*str = '\0';
			}
			else {
				if (SCRIPT(r)) {
					...
					if (vd)
						...
						*str = '\0';
						sprintf(buf2, 
									 "Trigger: %s, VNum %d. unknown room field: '%s'",
									 GET_TRIG_NAME(trig), GET_TRIG_VNUM(trig), field);
						script_log(buf2);
					}
				} else {
		 ...
		 int type, char *cmd)
{
	...
	*uid = '\0';
	...
	if (atoi(arg)!=0) { /* easy, if you pass an id number */
		...
		if (!name || !*name) {
			sprintf(buf2, "Trigger: %s, VNum %d. makeuid needs name: '%s'",
			GET_TRIG_NAME(trig), GET_TRIG_VNUM(trig), cmd);
			script_log(buf2);
			return;
		}
		...
		if (is_abbrev(arg, "mob")) {
			...
			switch (type) {
				...
				case OBJ_TRIGGER:
					...
					break;
				case MOB_TRIGGER:
					c = get_char_room_vis((struct char_data *)go, name, NULL, 0);
					break;
			}
			if (c) sprintf(uid,"%c%ld",UID_CHAR, GET_ID(c));
			...
			if (o) sprintf(uid,"%c%ld",UID_CHAR, GET_ID(o));
		} else if (is_abbrev(arg, "room")) {
			room_rnum r = NOWHERE;
			switch (type) {
				case WLD_TRIGGER:
					/* use %self.id% if you want this ..*/
					break;
				case OBJ_TRIGGER:
			...
			if (r != NOWHERE) sprintf(uid,"%c%ld",UID_CHAR, (long)r+ROOM_ID_BASE);
		} else {
			sprintf(buf2, "Trigger: %s, VNum %d. makeuid syntax error: '%s'",
			GET_TRIG_NAME(trig), GET_TRIG_VNUM(trig), cmd);
			script_log(buf2);
			return;
		}
	...
	for (cl = (mode == TRIG_NEW) ? trig->cmdlist : trig->curr_state;
		...
		if (*p == '*') /* comment */
			...
			if (cl->original && process_if(orig_cmd + 6, go, sc, trig,
				...
				if (GET_TRIG_LOOPS(trig) == 100) {
					...
					sprintf(buf,"SCRIPTERR: Trigger VNum %d has looped 100 times!!!",
						GET_TRIG_VNUM(trig));
					mudlog(buf, NRM, RIGHTS_TRIGGERS, TRUE);
					free(buf);
				}
			}
			...
			if (!strn_cmp(cmd, "eval ", 5))
				...
				mudlog(DG_SCRIPT_VERSION, NRM, RIGHTS_TRIGGERS, TRUE);
					...
					void *go, struct script_data *sc, int type, char *cond)
{
	...
	for (c = cl->next; c->next; c = c->next) {
		...
		if (!strn_cmp("while ", p, 6) || !strn_cmp("switch", p, 6))
			...
			sprintf(printbuf, "(%s) == (%s)", cond, p + 5);
			if (process_if(printbuf, go, sc, trig, type)) {
#else	/* new! improved! bug fixed! */
			eval_op("==", result, p + 5, printbuf, go, sc, trig);
			if (*printbuf && *printbuf!='0') {
#endif
...
/* additional commands added below */
...
void change_value(struct char_data *ch, char *argument)
{
	...
	if (!*buf2 || !isdigit(*buf2)) {
		mlog("SYSERR: Invalid entry in change_value (%s)", argument);
		return;
	}
	amount = atoi(buf2);
	...
	} else if (is_abbrev(buf1, "gold")) {
		GET_GOLD(ch) += amount;
	} else if (is_abbrev(buf1, "qp")) {
		GET_QP(ch) += amount;
		if (GET_QP(ch) < 0)
			GET_QP(ch) = 0;
	} else {
		mlog("SYSERR: Unknown change_value type (%s)", buf1);
	}
}
