...
************************************************************************ */
/* $Id: mail.c,v 1.16 2002/11/06 18:55:47 arcanere Exp $ */
...
int mail_recip_ok(char *name)
{
	...
	CREATE(recipient, struct char_data, 1);
	clear_char(recipient);
	CREATE(recipient->player_specials, struct player_special_data, 1);
	if (load_char(name, recipient) > -1) {
		char_to_room(recipient, 0);
		if (!PLR_FLAGGED(recipient, PLR_DELETED))
			ret = TRUE;
		extract_char_final(recipient);
	} else 
		free(recipient);
	return ret;
}
...
mail_index_type	*find_char_in_index(long searchee)
{
	...
	if (searchee < 0) {
		extended_mudlog(BRF, SYSL_BUGS, TRUE, "Mail system -- non fatal error #1 (searchee == %ld).", searchee);
		return (NULL);
	}
...
void write_to_file(void *buf, int size, long filepos)
{
	...
	if (filepos % BLOCK_SIZE) {
		extended_mudlog(BRF, SYSL_BUGS, TRUE, "Mail system -- fatal error #2!!! (invalid file position %ld)", filepos);
		no_mail = TRUE;
		return;
	}
	if (!(mail_file = fopen(MAIL_FILE, "r+b"))) {
		extended_mudlog(BRF, SYSL_BUGS, TRUE, "Unable to open mail file '%s'.", MAIL_FILE);
		no_mail = TRUE;
		return;
	}
...
void read_from_file(void *buf, int size, long filepos)
{
	...
	if (filepos % BLOCK_SIZE) {
		extended_mudlog(BRF, SYSL_BUGS, TRUE, "Mail system -- fatal error #3!!! (invalid filepos read %ld)", filepos);
		no_mail = TRUE;
		return;
	}
	if (!(mail_file = fopen(MAIL_FILE, "r+b"))) {
...
void index_mail(long id_to_index, long pos)
{
	...
	if (id_to_index < 0) {
		extended_mudlog(BRF, SYSL_BUGS, TRUE, "Mail system -- non-fatal error #4. (id_to_index == %ld)", id_to_index);
		return;
	}
...
int	scan_file(void)
{
	...
	file_end_pos = ftell(mail_file);
	fclose(mail_file);
	mlog("   %ld bytes read.", file_end_pos);
	if (file_end_pos % BLOCK_SIZE) {
		extended_mudlog(BRF, SYSL_BUGS, TRUE, "Error booting mail system -- Mail file corrupt!");
		extended_mudlog(BRF, SYSL_BUGS, TRUE, "Mail disabled!");
		return (0);
	}
	mlog("   Mail file read -- %d messages.", total_messages);
	return (1);
}				/* end of scan_file */
...
void store_mail(long to, long from, char *message_pointer)
{
	...
	if (to == -2) {	// "imms" - replacement of the imm_store_mail function
		for (i = 0; i <= top_of_p_table; i++) {
			if (player_table[i].rights >= RIGHTS_IMMORTAL)
				store_mail(player_table[i].id, from, message_pointer);
		}
		return;
	}
	...
	if (from < 0 || to < 0 || !*message_pointer) {
		extended_mudlog(BRF, SYSL_BUGS, TRUE, "Mail system -- non-fatal error #5. (from == %ld, to == %ld)", from, to);
		return;
	}
...
char *read_delete(long recipient)
{
	...
	if (recipient < 0) {
		extended_mudlog(BRF, SYSL_BUGS, TRUE, "Mail system -- non-fatal error #6. (recipient: %ld)", recipient);
		return (NULL);
	}
	if (!(mail_pointer = find_char_in_index(recipient))) {
		extended_mudlog(BRF, SYSL_BUGS, TRUE, "Mail system -- post office spec_proc error?  Error #7. (invalid character in index)");
		return (NULL);
	}
	if (!(position_pointer = mail_pointer->list_start)) {
		extended_mudlog(BRF, SYSL_BUGS, TRUE, "Mail system -- non-fatal error #8. (invalid position pointer %p)", position_pointer);
		return (NULL);
	}
	...
	if (header.block_type != HEADER_BLOCK) {
		extended_mudlog(BRF, SYSL_BUGS, TRUE, "Oh dear. (Header block %ld != %d)", header.block_type, HEADER_BLOCK);
		no_mail = TRUE;
		extended_mudlog(BRF, SYSL_BUGS, TRUE, "Mail system disabled!  -- Error #9. (Invalid header block.)");
		return (NULL);
	}
	...
	*(tmstr + strlen(tmstr) - 1) = '\0';
	...
	sprintf(buf, " * * * * Arcane Realms Mail System * * * *\r\n"
		"Date: %s\r\n"
		"  To: %s\r\n"
		"From: %s\r\n\r\n", tmstr, to ? to : "Unknown",
		from ? from : "Unknown");
				...
				int cmd, char *arg)
{
	...
	if (GET_GOLD(ch) < STAMP_PRICE && !IS_IMMORTAL(ch)) {
		sprintf(buf, "$n tells you, 'A stamp costs %d coin%s.'\r\n"
			"$n tells you, '...which I see you can't afford.'", STAMP_PRICE,
						STAMP_PRICE == 1 ? "" : "s");
		act(buf, FALSE, mailman, 0, ch, TO_VICT);
		return;
	}
	if ((!(str_cmp(buf, "all")) || !(str_cmp(buf, "imms"))) && !IS_IMPL(ch)) {
		act("$n tells you, 'You must be an Implementor to send a message to everyone or all immortals!'",
				FALSE, mailman, 0, ch, TO_VICT);
		return;
	}
	...
	if (!(str_cmp(buf, "all"))) {
		recipient = -1;
	} else if (!(str_cmp(buf, "imms"))) {
		recipient = -2;
	} else if ((recipient = get_id_by_name(buf)) < 0 || !mail_recip_ok(buf)) {
	...
	string_write(ch->desc, write, MAX_MAIL_SIZE, recipient, EDIT_MAIL);
}
				...
				int cmd, char *arg)
{
	...
	while (has_mail(GET_IDNUM(ch))) {
		...
		GET_OBJ_EXTRA(obj) = ITEM_UNIQUE_SAVE;
	...
	if (mailcount > 0) {
		sprintf(buf, "$n gives you a %s of mail.", mailcount > 1 ? "heap" : "piece");
		act(buf, FALSE, mailman, 0, ch, TO_VICT);
		sprintf(buf, "$N gives $n a %s of mail.", mailcount > 1 ? "heap" : "piece");
		act(buf, FALSE, ch, 0, mailman, TO_ROOM);
	}
}


void receive_mail(struct char_data *ch)
{
	char buf[256];
	struct obj_data *obj;
	int mailcount = 0;
	...
	if (mailcount > 0) {
		sprintf(buf, "You receive a %s of mail.", mailcount > 1 ? "heap" : "piece");
		act(buf, FALSE, ch, 0, 0, TO_CHAR);
		sprintf(buf, "$n receives a %s of mail.", mailcount > 1 ? "heap" : "piece");
