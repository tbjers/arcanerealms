...
***********************************************************************	*/
/* $Id: alias.c,v 1.4 2002/03/28 11:53:59 arcanere Exp $ */
...
void write_aliases(struct char_data *ch)
{
	...
	if ((file = fopen(fn, "w")) == NULL) {
		extended_mudlog(BRF, SYSL_BUGS, TRUE, "Couldn't save aliases for %s in '%s'.", GET_NAME(ch), fn);
...
{		
	...
	if ((file = fopen(xbuf, "r")) == NULL) {
		if (errno != ENOENT) {
			extended_mudlog(BRF, SYSL_BUGS, TRUE, "Couldn't open alias file '%s' for %s.", xbuf, GET_NAME(ch));
