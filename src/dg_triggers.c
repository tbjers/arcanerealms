	...
	if (SCRIPT_CHECK(ch, MTRIG_ACT) && !AFF_FLAGGED(ch, AFF_CHARM) &&
		...
		for (t = TRIGGERS(SCRIPT(ch)); t; t = t->next)  {
			...
			if (((GET_TRIG_NARG(t) && word_check(str, GET_TRIG_ARG(t))) ||
				...
				script_driver((char_data *)ch, t, MOB_TRIGGER, TRIG_NEW);
				break;
			}        
		}
}
...
int	cast_mtrigger(char_data *ch, char_data *actor, char *targ, struct spell_info_type *spell)
{
	trig_data *t;
	char buf[MAX_INPUT_LENGTH];
	...
	for (t = TRIGGERS(SCRIPT(ch)); t; t = t->next) {
		if (TRIGGER_CHECK(t, MTRIG_CAST) &&
			...
			ADD_UID_VAR(buf, t, actor, "actor", 0);
			skip_spaces(&targ);
			add_var(&GET_TRIG_VARS(t), "arg", targ, 0);
			sprintf(buf, "%d", spell->number);
			add_var(&GET_TRIG_VARS(t), "spell", buf, 0);
			sprintf(buf, "%d", spell->skill);
			add_var(&GET_TRIG_VARS(t), "spell_skill", buf, 0);
			sprintf(buf, "%s", spell->name);
			add_var(&GET_TRIG_VARS(t), "spellname", buf, 0);
			sprintf(buf, "%s", spell->long_name);
			add_var(&GET_TRIG_VARS(t), "spell_long_name", buf, 0);
			return script_driver(ch, t, MOB_TRIGGER, TRIG_NEW);
		}
	}
...
int	cast_otrigger(obj_data *obj, char_data *actor, char *targ, struct spell_info_type *spell)
{
	trig_data *t;
	char buf[MAX_INPUT_LENGTH];
...
int	cast_wtrigger(char_data *actor, char_data *vict, obj_data *obj, char *targ, struct spell_info_type *spell)
{
	room_data *room;
	trig_data *t;
	char buf[MAX_INPUT_LENGTH];
	...
	for (t = TRIGGERS(SCRIPT(room)); t; t = t->next) {
		if (TRIGGER_CHECK(t, WTRIG_CAST) &&
			...
			ADD_UID_VAR(buf, t, actor, "actor", 0);
			if (vict)
				ADD_UID_VAR(buf, t, vict, "vict", 0);
			if (obj)
				ADD_UID_VAR(buf, t, obj, "obj", 0);
			skip_spaces(&targ);
			add_var(&GET_TRIG_VARS(t), "arg", targ, 0);
			sprintf(buf, "%d", spell->number);
			add_var(&GET_TRIG_VARS(t), "spell", buf, 0);
